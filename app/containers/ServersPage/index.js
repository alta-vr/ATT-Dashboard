...
import { Grid, Menu, Segment, Input, Form, Button, Message, Card, Icon, Feed, List, Image, Divider } from 'semantic-ui-react'
...
export function ServersPage({servers, getServers, connect, disconnect}) {
  ...
  console.log(RemoteConsoles.consoles);
  ...
  return (
    <div>
      {!!servers.error && <Message
        error
        header='Error!'
        content={servers.error}
      />}
      <Menu>
        <Menu.Item>
          <Button primary onClick={() => getServers()}>Refresh</Button>
          <Button onClick={() => connect(5, "127.0.0.1", 1758, 1759)}>Test</Button>
        </Menu.Item>
      </Menu>
      <Card.Group>
        {servers.servers && servers.servers.map(item => (
        <Card key={item.id}>
          <Card.Content>
            <Card.Header>{item.name}</Card.Header>
            <Card.Meta>{GameVersion.toStringNoStream(item.meta_data.version)}</Card.Meta>
            <Divider/>
            <Grid columns={2}>
...
function mapDispatchToProps(dispatch) {
  return {
    dispatch,
    getServers: () => { dispatch(Servers.getServers()); }, 
    connect: (id, ip, commandPort, loggingPort) => { dispatch(RemoteConsoles.connect(id, ip, commandPort, loggingPort)); },
    disconnect: (id) => { dispatch(RemoteConsoles.disconnect(id)); }
  };
}
